...
RSpec.describe 'pocket_monsters index page' do
  ...
  it 'shows all of the pocket_monsters' do 
    ...
    expect(page).to have_content(@pikachu.can.evolve)
    expect(page).to have_content(@caterpie.can.evolve)
    expect(page).to have_content(@bulbasaur.can.evolve)
    expect(page).to have_content(@growlithe.can.evolve)
    expect(page).to have_content(@charmeleon.can.evolve)
    expect(page).to have_content(@haunter.can.evolve)
    expect(page).to have_content(@blastoise.can.evolve)
    expect(page).to have_content(@mewtwo.can.evolve)
    expect(page).to have_content(@starmie.can.evolve)
    expect(page).to have_content(@togepi.can.evolve)
    expect(page).to have_content(@charizard.can.evolve)
    expect(page).to have_content(@dragonite.can.evolve)
  end
end